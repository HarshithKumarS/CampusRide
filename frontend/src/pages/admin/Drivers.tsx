...
interface Driver {
  ...
  name: string;
  phone?: string;
  roll_number?: string; // Reused for license number
  created_at: string;
}
...
const Drivers: React.FC = () => {
  ...
  const handleOpenEdit = (driver: Driver) => {
    ...
    setLicenseNumber(driver.roll_number || '');
    setPassword('');
    setError('');
    setModalOpen(true);
  };
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (editingDriver) {
        // Edit API
        const res = await api.put(`/drivers/${editingDriver.id}`, {
          name,
          email,
          phone,
          roll_number: licenseNumber,
          password: password || undefined
        });
        setDrivers(drivers.map(d => d.id === editingDriver.id ? res.data : d));
      } else {
        // Create API
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-3xl font-extrabold tracking-tight">Driver Registry</h1>
          <p className="text-slate-500 dark:text-slate-400">View, register, and modify active university transit officers.</p>
        </div>
        <button
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl overflow-hidden shadow-sm space-y-4">
        ...
        <div className="p-6 border-b border-slate-200 dark:border-slate-800">
          <div className="relative max-w-md">
            ...
            <input
              type="text"
              placeholder="Search by officer name, email, or license..."
              value={searchTerm}
              ...
              {filteredDrivers.length > 0 ? (
                filteredDrivers.map((d) => (
                  <tr key={d.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-950/20">
                    <td className="px-6 py-4 whitespace-nowrap font-semibold">{d.name}</td>
                    <td className="px-6 py-4 whitespace-nowrap font-mono font-medium">{d.roll_number || 'N/A'}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-slate-500 dark:text-slate-400">{d.email}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-6 bg-black/50 backdrop-blur-sm">
          <div className="w-full max-w-lg bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-2xl p-8 space-y-6 relative max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex justify-between items-center pb-4 border-b border-slate-100 dark:border-slate-800">
              <h3 className="text-xl font-bold">{editingDriver ? 'Edit Driver Details' : 'Register New Driver'}</h3>
            ...
            <form onSubmit={handleSave} className="space-y-4">
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-bold text-slate-500 uppercase">
                  {editingDriver ? 'Reset Password (Leave blank to keep current)' : 'Password'}
                </label>
                <input
                  type="password"
